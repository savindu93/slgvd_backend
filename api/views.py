...
def process_upload(self, job_id, group_name):

    
    print("In process_upload")
...
# Saves tsv files of variant annotations in MEDIA_ROOT and
# returns a filepath
def save_file(uploaded_files):

    filepaths = []

    # uploaded_file is request.FILES["files"]
    # choose a subfolder inside MEDIA_ROOT
    for uploaded_file in uploaded_files:
        ...
        filepaths.append(default_storage.path(filename))
        
    # This returns a relative path like: "uploads/myfile.tsv"
    # Convert to absolute path so Celery can open it easily
    return filepaths
...
class DataUpload(APIView):
    ...
    def post(self, request, format=None):
        ...
        # Create a Job ID
        job = UploadJob.objects.create(status = "queued", progress = 0, file_path=paths, file_type = fileType, username = user.username)
        ...
        #                         # Create new entries or update existing entries
        #                         def create_update_entries(var_id, variant_object, freq_object, sub_id, progress, no_freqs_updated):
        ...
        #                         no_freqs_updated = create_update_entries(var_id, variant_object, freq_object, sub_id, progress, no_freqs_updated)
        ...
        #                             no_freqs_updated = create_update_entries(var_id, variant_object_1, freq_object_1, sub_id, progress, no_freqs_updated)
